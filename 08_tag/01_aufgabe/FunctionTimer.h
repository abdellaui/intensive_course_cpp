...
public:
	...
	FunctionTimer(T function, double& time) : function(function), time(time), stop()
	{
...
private:
	double &time;
	decltype(std::chrono::system_clock::now()) start, stop;
};
...
{
public:
	FunctionTimer(double& time) : time(time), stop()
	{
		start = std::chrono::system_clock::now();
	}
